...
import ModalMantineDinamico from "components/ModalDinamico";
...
export function Sidebar({ setarIsLogged }: SideBarProps) {
  ...
  return (
    <Navbar className={styles.sidebar}>
      ...
      {isUserLogged ? (<Navbar.Section>
        <Stack justify="center" spacing={0}>
          {/* <Avatar className={styles.buttonCart__avatar} /> */}
          {/* <NavbarLink
            icon={IconLogout}
            label="Sair"
            onClick={() => setOpenModalLogoutQuestion(true)}
          /> */}
          <ModalDinamico setarIsLogged={setarIsLogged} />
        </Stack>
      </Navbar.Section>) : " "}
